...
//old codes June 2, 2024
  ...
  router.post("/", uploadOptions.single('image'), async (require, response) => {
    try {
      const { firstName, lastName, middleName, mobileNumber, email, password } = require.body;
  
      if (!firstName || !lastName || !middleName || !mobileNumber || !email || !password) {
        return response.status(400).send({
          message: "Send all required fields: firstName, lastName, middleName, mobileNumber, email, password",
        });
      }
      ...
      if (!require.file) {
        ...
        const result = await cloudinary.v2.uploader.upload(require.file.path, cloudinaryFolderOption);
        avatar = {
